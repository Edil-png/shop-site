...
export default function AdminUsersPage() {
  ...
  async function deleteUsers(id: string) {
    try {
      ...
      console.error("Не удалось удалить пользователь");
    }
  }
  return (
    <div className="space-y-6 pb-10">
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl shadow-sm overflow-hidden">
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span
                        className={`text-sm ${user.status === "active" ? "text-emerald-700 dark:text-emerald-400" : "text-rose-700 dark:text-rose-400"}`}
                      >
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button
                        className="p-2 hover:bg-blue-50 dark:hover:bg-blue-900/30 text-gray-400 hover:text-blue-600 rounded-lg transition-all"
                        title="Редактировать"
                      >
                      ...
                      <button
                        ...
                        className="p-2 hover:bg-rose-50 dark:hover:bg-rose-900/30 text-gray-400 hover:text-rose-600 rounded-lg transition-all"
                        title="Удалить"
                      >
        ...
        <div className="p-4 border-t border-gray-100 dark:border-gray-700 flex items-center justify-between bg-gray-50/30 dark:bg-gray-900/10">
          ...
          <div className="flex gap-2">
            <button
              className="px-3 py-1 border border-gray-200 dark:border-gray-700 rounded-md text-xs disabled:opacity-50"
              disabled
            >
              Назад
            </button>
            <button className="px-3 py-1 border border-gray-200 dark:border-gray-700 rounded-md text-xs">
              Вперед
            </button>
          </div>
        </div>
...
function getRoleBadge(role: string) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-[11px] font-bold uppercase tracking-tight ${config.class}`}
    >
