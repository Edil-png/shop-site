...
export default function AdminProductsPage() {
  const { categories, products, setProducts } = useProducts(); // Предполагаем, что setProducts есть в контексте
  ...
  // Функция для удаления товара из списка (без рендеринга/перезагрузки)
  const handleProductDeleted = (id: string) => {
    // Если в контексте есть setProducts, вызываем его
    if (setProducts) {
    ...
    <div className="space-y-6 pb-20">
      ...
      <div className="flex flex-col md:flex-row gap-3">
        ...
        <select
          ...
          className="px-4 py-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl text-sm font-medium outline-none focus:border-blue-500 cursor-pointer"
        >
          {categories.map((category) => (
            <option key={category.id} value={category.slug}>
              {category.name === "all" ? "Все категории" : category.name}
            </option>
          ))}
        </select>
      </div>
      ...
      <TableCard
        ...
        onDelete={handleProductDeleted} // Теперь функция реализована
      />
        ...
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 animate-in fade-in slide-in-from-bottom-4 duration-300">
          <div className="bg-gray-900 text-white px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-6 border border-white/10 ring-4 ring-black/5">
            <div className="flex items-center gap-3 pr-6 border-r border-white/20">
              <div className="h-6 w-6 bg-blue-500 rounded-md flex items-center justify-center text-[10px] font-bold">
                {selectedProducts.length}
              </div>
              <span className="text-sm font-medium whitespace-nowrap">Выбрано</span>
            </div>
            <div className="flex items-center gap-4">
              <button className="flex items-center gap-2 text-sm text-rose-400 hover:text-rose-300 transition-colors font-medium">
                <Trash2 className="h-4 w-4" /> Удалить {selectedProducts.length > 1 ? 'все' : ''}
              </button>
              <button
...
// Вспомогательная функция (getStatusBadge) остается без изменений
const getStatusBadge = (status: string) => {
  ...
  const config = map[status as keyof typeof map] || map["in-stock"];
